...
void f3 ( u8 k[16], u8 rand[16], u8 ck[16])
{
#ifdef showmethod
	printf("ffunction: f3\n");
#endif
...
uint64_t clock_gettime_nsec_np_1(clockid_t clock_id)
{
    switch(clock_id){
    case CLOCK_REALTIME: {
        struct timespec tv;
    ...
    case CLOCK_PROCESS_CPUTIME_ID: {
		struct timespec tv;
        int ret = clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &tv);
        if (ret) 
			return 0;
        return timeval2nsec(tv);
    }
